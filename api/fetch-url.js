...
function doFetch(url, redirects) {
  return new Promise(function(resolve, reject) {
    ...
    var parsed;
    try { parsed = new URL(url); } catch(e) { return resolve('[URL invalide]'); }
    ...
    var req = lib.request(opts, function(response) {
      ...
      if (ct.indexOf('pdf') > -1 || url.toLowerCase().indexOf('.pdf') > -1) return resolve('[PDF: ' + url + ']');
      var data = '';
      response.setEncoding('utf8');
      response.on('data', function(chunk) { data += chunk; if (data.length > 200000) response.destroy(); });
      response.on('end', function() { resolve(clean(data, url)); });
